...
expected_bias_sqrt <- -1 / (8 * sqrt(mu)) + 1 / (16 * mu ^ (1.5))
observed_bias_sqrt <- mean(sqrt(sim_df_tr_loess$y) - sqrt(sim_df_loess$y))
cat("On the square root scale, a Taylor expansion estimates a bias of",
    expected_bias_sqrt, "\nwhereas the mean difference in loess estimates",
    "(on the sqrt scale) is", observed_bias_sqrt, "\n")

# Analysis of bias on the original scale --------------------------------------
E_sqrt <- sqrt(mu) + expected_bias_sqrt
E_reconstituted <- E_sqrt ^ 2
expected_bias <- E_reconstituted - mu
observed_bias <- mean(sim_df_tr_loess$y - sim_df_loess$y)
cat("On the original scale, a Taylor expansion estimates a bias of",
    expected_bias, "\nwhereas the mean difference in loess estimates",
    "is", observed_bias, "\n")
